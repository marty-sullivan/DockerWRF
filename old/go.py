...
try:
    ...
    error('Python paramiko module is not installed. Try `pip install paramiko` or `easy_install paramiko` (preferrably in a virtual_env).')
    exit(1)
...
def create_aws():
    ...
    # IAM Role
    r['aws']['roles'] = [r['iam'].create_role(Path=r['iam_path'], RoleName=r['prefix'] + 'role', AssumeRolePolicyDocument=ECS_ASSUME_ROLE_POLICY_DOC)]
    r['aws']['roles'][0].attach_policy(PolicyArn=ECS_ROLE_POLICY_ARN)
        
    # IAM Instance Profile
    r['aws']['instance_profiles'] = [r['iam'].create_instance_profile(Path=r['iam_path'], InstanceProfileName=r['prefix'] + 'profile')]
    r['aws']['instance_profiles'][0].add_role(RoleName=r['aws']['roles'][0].role_name)

    # Security Group
    r['aws']['security_groups'] = [r['aws']['subnets'][0].vpc.create_security_group(GroupName=r['prefix'] + 'sg', Description=r['prefix'] + 'sg')]
    ...
    # Placement Group
    r['aws']['placement_groups'] = [r['ec2'].create_placement_group(GroupName=r['prefix'] + 'placement', Strategy='cluster')]

    # Key Pair
    r['aws']['key_pairs'] = [r['ec2'].create_key_pair(KeyName=r['prefix'] + 'keypair')]
    r['rsa_keys'] = ['./keys/' + r['aws']['key_pairs'][0].key_name + '.pem']
    with open(r['rsa_keys'][0], 'w') as pem:
        pem.write(r['aws']['key_pairs'][0].key_material)
        chmod(r['rsa_keys'][0], 0400)

    # ECS Cluster
    r['aws']['clusters'] = [r['ecs'].create_cluster(clusterName=r['prefix'] + 'cluster')]
...
def destroy_aws():
    ...
    response = raw_input('--> ')
    if response != 'DESTROY':
        print('Destroy Request Cancelled.')
        exit(0)
